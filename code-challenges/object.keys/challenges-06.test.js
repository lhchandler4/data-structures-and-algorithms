...
const totalCharacters = (arr) => {
  let howmuch= [];
  arr.forEach(obj=>{
    howmuch.push(obj.name);
    if(obj.spouse!==null&&howmuch.includes(obj.spouse)===false){
      howmuch.push(obj.spouse);
    }
    if(obj.children!==[]){
      for (let i=0;i<obj.children.length;i++){
        if(howmuch.includes(obj.children[i])===false){
          howmuch.push(obj.children[i]);}
      }
    }
  })
