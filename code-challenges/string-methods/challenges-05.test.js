...
const listFoods = (recipe) => {
  ...
  for(let i=0; i<recipe.ingredients.length; i++){
    let ohh = recipe.ingredients[i];
    result.push(ohh.slice(ohh.indexOf(' ', (ohh.indexOf(' ') + 1))+1));
  }
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  if (numberOfCharacters>str.length){
    ...
    let arr = str.split('');
    let abc = arr.splice(arr.length-numberOfCharacters,numberOfCharacters);
    let answer = arr.join();
    let blah = answer.replace(/,/gi, '');
    return blah;
  }else{
    return str;
  }
