...
const getStatName = (arr, minBaseStat) => {
  let nameOfStat = arr.filter(function (i) {
    return i.baseStat > minBaseStat;
  });
  return nameOfStat.map(i=>i.stat.name);
};
...
const evenOddNumericValues = (arr) => {
  let evenAndOdd = [];
  let justNumbers = arr.filter(n=> typeof(n)==='number');
  justNumbers.forEach(function(n){
    if(n % 2===1){
      evenAndOdd.push('odd');
    }else{evenAndOdd.push('even');}
  })
  return evenAndOdd;
};
