...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i=0; i<arr.length; i++){
    aNewArray.push(Math.pow(2,arr[i]));
  }
  return aNewArray;
};
...
const forEachTwoToThe = (arr) => {
  let aNewArray = [];
  arr.forEach(potato => {
    aNewArray.push(Math.pow(2, potato));
  });
  return aNewArray;
};
...
const mapTwoToThe = (arr) => {
  let secondPower = arr.map( i => Math.pow(2,i));
  return secondPower;
};
...
const charCode = (arr) => {
  let needed = arr.map( i => i.charCodeAt());
  return needed;
};
...
const evenOdd = (arr) => {
  let evenOrOdd = arr.map(function(i){
    if(typeof(i)!=='number'){
      return 'N/A'
    }else if(i % 2===0){
      return 'even';
...
const extractAbilities = (arr) => {
  let abilityName = arr.map((i)=> {return i.ability.name});
  return abilityName;
};
...
const extractStats = (arr) => {
  let statistics = arr.map(function (obj) {
    return {
      name: obj.stat.name,
